...
namespace PodcastReader.Infrastructure.Caching
{
    public class CachingStateVm : ReactiveObject, ICachingState
    {
        ...
        public CachingStateVm(IReactiveProgress<ProgressValue> progress)
        {
            ...
            this.WhenAny(x => x.FinalSize, x => x).Subscribe(ch => Debug.WriteLine("FinalSize: " + ch.Value));
            this.WhenAny(x => x.CachedSize, x => x).Subscribe(ch => Debug.WriteLine("CachedSize: " + ch.Value));
        }
